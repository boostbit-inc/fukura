    ...
    for time_format in time_formats {
        ...
        if !output.status.success() {
            ...
            assert!(!stderr.to_lowercase().contains("invalid format"),
                    "Format should be valid: {}", time_format);
        }
    }
}
...
fn test_missing_required_arguments() {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        assert!(stderr.to_lowercase().contains("required") ||
...
fn test_empty_repository_commands() {
    ...
    for cmd in safe_commands {
        ...
        assert!(output.status.success(), 
                "Command {:?} should work with empty repo", cmd);
    }
}
...
fn test_special_refs() {
    ...
    for ref_str in refs {
        let output = Command::new(&binary_path)
            ...
            .current_dir(temp_dir.path())
            .output()
            .expect(&format!("Failed to view {}", ref_str));
...
fn test_helpful_error_messages() {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        // Should mention init
        assert!(stderr.to_lowercase().contains("init") ||
                stderr.to_lowercase().contains("repository"));
    }
}
...
fn test_complete_dev_workflow() {
    ...
    // 3. Add some notes
    let _add = Command::new(&binary_path)
        .args(["add", "--title", "Implementation notes", "--body", "Details", "--no-editor"])
        .current_dir(temp_dir.path())
        .output()
        .expect("Failed to add note");
...
fn test_various_time_expressions() {
    ...
    let valid_times = vec![
        "1m ago", "5m ago", "30m ago", "1h ago", "2h ago",
        "1h 30m ago", "2h 15m ago", "45m ago",
        "1m", "2h", "30m",  // Without "ago"
    ];
    ...
    for time_expr in valid_times {
        ...
        if !output.status.success() {
            ...
            assert!(!stderr.to_lowercase().contains("invalid format"),
                    "Time expression should be valid: {}", time_expr);
        }
    }
}
...
fn test_json_output() {
    ...
    // Add note
    let _add = Command::new(&binary_path)
        .args(["add", "--title", "JSON test", "--body", "Content", "--no-editor"])
        .current_dir(temp_dir.path())
        .output()
