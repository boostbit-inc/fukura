...
"#;
    ...
    let loaded_config = FukuraConfig::load(&config_path)
...
fn test_save_and_reload_config_with_recording() {
    ...
    let mut config = FukuraConfig::default();
    config.version = 1;
    ...
    // Reload and verify
    let reloaded_config = FukuraConfig::load(&config_path)
...
"#;
    ...
    let loaded_config = FukuraConfig::load(&config_path)
        .expect("Failed to load config with extreme values");
...
"#;
    ...
    let result = FukuraConfig::load(&config_path);
    assert!(result.is_err(), "Should fail to load config with wrong types");
}
...
fn test_recording_config_serialization() {
    ...
    let serialized = serde_json::to_string(&config)
        .expect("Should serialize recording config");
    
    let deserialized: RecordingConfig = serde_json::from_str(&serialized)
        .expect("Should deserialize recording config");
...
fn test_reasonable_config_limits() {
    ...
    assert!(min_in_hours < max_in_hours,
