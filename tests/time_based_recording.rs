...
fn setup_test_repo() -> TempDir {
    ...
    assert!(
        ...
        "Failed to initialize test repository"
    );
    temp_dir
}
...
fn test_time_based_rec_invalid_format() {
    ...
    let invalid_formats = vec!["invalid", "3x ago", "abc", "3.5h ago", "-1m ago"];
    ...
    for invalid_format in invalid_formats {
        ...
        assert!(
            !output.status.success(),
            "Should have failed with invalid format: {}",
            invalid_format
        );

        let stderr = String::from_utf8_lossy(&output.stderr);
        assert!(
            stderr.to_lowercase().contains("invalid") || stderr.to_lowercase().contains("format"),
...
fn test_time_based_rec_case_insensitive() {
    ...
    let case_variations = vec!["3m ago", "3M ago", "3M AGO", "3m AGO"];
    ...
    for format in case_variations {
        ...
        if !output.status.success() {
            ...
            assert!(
                ...
                "Should not fail format parsing for: {}. Got: {}",
                format,
                stderr
            );
        }
    ...
    if output1.status.success() {
        ...
        if !output2.status.success() {
            ...
            assert!(
                ...
                "Should mention existing recording. Got: {}",
                stderr
            );
        }
    }
...
"#;
    ...
    if !output.status.success() {
        ...
        assert!(
            !stderr.to_lowercase().contains("maximum") || !stderr.to_lowercase().contains("hours"),
            "Should not fail time validation with custom config. Got: {}",
            stderr
        );
    }
}
...
fn test_time_based_rec_help_examples() {
    ...
    assert!(
        stdout.contains("rec \"Task description\""),
...
fn test_time_based_rec_full_integration() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("recording started") || stdout.contains("Time-based recording"),
    ...
    assert!(
        status_stdout.contains("Recording in progress"),
